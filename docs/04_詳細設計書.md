...
flowchart TD
    ...
    D --> E["各タスクを順に配置"]
    E --> F["effectiveDuration算出<br/>（過去開始は経過+残作業）"]
...
- `effectiveDuration(task, eTs)`
  - `eTs >= now` → 制作時間そのまま。
  - `eTs < now` → `経過稼働時間(workingHoursBetweenTs) + 残作業時間(hours - completedHours)`。
  - これにより進捗に応じて終了予定が前後する（要件F-08）。
...
| `advanceByHours / applyPendingAdvance / advanceToday` | 経過自動加算（担当者カスケード）。 |
...
| `CalendarView` | 1日/週間は横タイムライン（残業で時間軸延長）、月間/全期間は午前/午後2分割セル。現在時刻ライン表示。 |
