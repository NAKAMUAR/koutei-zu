...
// ---- ルール1: alert() 禁止（トースト通知 notify() を使う） ----
for (const f of files) {
  if (f.endsWith('ui/toast.jsx')) continue;
  const src = read(f);
...
for (const p of ['src/ui/toast.jsx', 'src/ui/controls.jsx', 'src/ui/useEscKey.js', 'src/views/MemoView.jsx', 'src/lib/text.js']) {
